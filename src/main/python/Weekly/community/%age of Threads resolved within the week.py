...
# # Initialize answered discussion count to 0
# answered_discussion_count = 0
...
while True:
    ...
    for discussion in data['data']['repository']['discussions']['nodes']:
        ...
        if start_date <= created_at <= end_date:
            discussion_count += 1
            print(
                f"Discussion created on {created_at} isLocked={discussion['locked']}")
...
percentage = (locked_discussion_count / discussion_count) * 100

print(f"Number of discussions created between {start_date} and {end_date}: {discussion_count}")
# print(f"Number of answered discussions created between {start_date} and {end_date}: {answered_discussion_count}")
print(f"Number of locked discussions created between {start_date} and {end_date}: {locked_discussion_count}")
print(f"Percentage of threads resolved within the week {start_date} and {end_date}: {percentage}%")
